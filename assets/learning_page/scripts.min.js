...
(function ($) {
 ...
 function handleContent(itemId, itemType) {
    ...
    $.post('/course/learning/itemInfo', data, function(result) {
        ...
        // تحديث حالة الأزرار بعد تحميل أي محتوى بنجاح
        updateNavigationButtons();
        updateLessonCompleteButton();

        // إعادة رسم الأيقونات إذا كانت موجودة
  ...
  function handleDownloadCertificateHtml(result) {
    ...
    var otherHtml = '';
    if (result && result !== '') {
      otherHtml = "\n                <a href=\"/panel/quizzes/results/".concat(result, "/showCertificate\" target=\"\" class=\"btn btn-primary btn-sm mt-15\">").concat(downloadLang, "</a>\n            ");
    } else {
  ...
  function handleCourseCertificateHtml(id) {
    ...
    var otherHtml = '';
    if (id && id !== '') {
      otherHtml = "\n                <a href=\"/panel/certificates/webinars/".concat(id, "/show\" target=\"\" class=\"btn btn-primary btn-sm mt-15\">").concat(showLang, "</a>\n            ");
    } else {
  ...
  function handleQuizHtml(quiz) {
    ...
    if (quiz.has_expired) {
      ...
      if (quiz.can_try) {
        otherHtml += "\n                <a href=\"/panel/quizzes/".concat(quiz.id, "/start\" target=\"\" class=\"btn btn-primary btn-sm mt-15\">").concat(quizPageLang, "</a>\n            ");
      } else {
  ...
  function handleLiveSessionHtml(session) {
    ...
    otherHtml += "\n            <div class=\"d-flex align-items-center mt-15\">\n                <a href=\"".concat(session.join_url, "\" target=\"\" class=\"btn btn-primary btn-sm \">").concat(joinTheClassLang, "</a>\n                <a href=\"").concat(courseUrl, "\" class=\"btn btn-white btn-sm ml-10\">").concat(coursePageLang, "</a>\n            </div>\n        ");
    var html = handleContentBoxHtml(title, hint, img, otherHtml, 'mt-10');
    learningPageContent.html(html);
  }
    ...
    if (file.online_viewer && file.online_viewer !== '0' || file.downloadable && file.downloadable !== '0') {
      if (file.online_viewer && file.online_viewer !== '0') {
        ...
        if (file.is_video) {
          ...
          var otherHtml = "<a href=\"".concat(courseUrl, "/file/").concat(file.id, "/download\" class=\"btn btn-primary btn-sm mt-15\" target=\"\">").concat(downloadLang, "</a>");
      ...
      switch (file.storage) {
        ...
        case 'upload_archive':
          ...
          var _otherHtml = "<a href=\"".concat(courseUrl, "/file/").concat(file.id, "/showHtml\" target=\"\" class=\"btn btn-primary btn-sm mt-15\">").concat(showLang, "</a>");
 ...
 function handleFileIframe(fileId) {
  $.post('/course/getFilePath', {
    ...
    if (result && result.code === 200) {
      var path = result.path; // الرابط أو المحتوى القادم من الخادم
      var finalContent; // متغير لتخزين المحتوى النهائي الذي سيتم عرضه

      // -- بداية التعديل --

      // 1. التحقق إذا كان الرابط هو رابط Google Drive للملفات
      if (path.startsWith('https://drive.google.com/file/d/')) {
        
        // 2. استبدال 'view' بـ 'preview' لجعله قابلًا للتضمين
        var embedUrl = path.replace('/view', '/preview');
        
        // 3. إنشاء وسم iframe كامل باستخدام الرابط الجديد
        finalContent = `<iframe src="${embedUrl}" width="100%" height="500px" allow="autoplay" frameborder="0" allowfullscreen="true"></iframe>`;

      } else {
        // 4. إذا لم يكن رابط Google Drive، استخدم المحتوى كما هو (الحالة الافتراضية)
        finalContent = path;
      }
      
      // -- نهاية التعديل --

      // استخدام المحتوى النهائي (finalContent) لعرضه في الصفحة
      var $iframeHtml = '<div class="learning-content-iframe">' + finalContent + '</div>';
...
// ========= بداية الكود الجديد: منطق أزرار التنقل =========

/**
 * تحديث حالة أزرار التالي والسابق وإظهارها أو إخفائها
 */
function updateNavigationButtons() {
    ...
    if (!activeItem.length) {
        $('#learning-navigation').addClass('d-none');
        return;
    }

    var activeIndex = allItems.index(activeItem);
    var prevBtn = $('#learning-previous-btn');
    var nextBtn = $('#learning-next-btn');
    var navContainer = $('#learning-navigation');

    // التعامل مع زر "السابق"
    if (activeIndex > 0) {
        var prevItem = $(allItems[activeIndex - 1]);
        prevBtn.attr('data-id', prevItem.data('id')).attr('data-type', prevItem.data('type'));
        prevBtn.prop('disabled', false);
        prevBtn.removeClass('d-none');
    } else {
        prevBtn.prop('disabled', true);
        prevBtn.addClass('d-none'); // إخفاء الزر إذا كان هذا هو العنصر الأول
    }

    // التعامل مع زر "التالي"
    if (activeIndex < (allItems.length - 1)) {
        var nextItem = $(allItems[activeIndex + 1]);
        nextBtn.attr('data-id', nextItem.data('id')).attr('data-type', nextItem.data('type'));
        nextBtn.prop('disabled', false);
        nextBtn.removeClass('d-none');
    } else {
        nextBtn.prop('disabled', true);
        nextBtn.addClass('d-none'); // إخفاء الزر إذا كان هذا هو العنصر الأخير
    }

    // إظهار الحاوية فقط إذا كان أحد الزرين على الأقل ظاهرًا
    if (prevBtn.is(':visible') || nextBtn.is(':visible')) {
        navContainer.removeClass('d-none');
    } else {
        navContainer.addClass('d-none');
    }
}
// ========= بداية كود أزرار التنقل والإكمال =========

/**
 * تحديث حالة أزرار التالي والسابق
 */
function updateNavigationButtons() {
    var allItems = $('.tab-item');
    var activeItem = $('.tab-item.active');
    ...
    if (!activeItem.length || !navContainer.length) {
        if (navContainer.length) navContainer.addClass('d-none');
        return;
    }
    ...
    // إظهار الحاوية إذا كان أي من الزرين ظاهراً
    if (prevBtn.is(':visible') || nextBtn.is(':visible')) {
        navContainer.removeClass('d-none');
    } else {
        navContainer.addClass('d-none');
...
/**
 * تحديث حالة زر إكمال الدرس (نسخة معدلة)
 */
function updateLessonCompleteButton() {
    var completeBtn = $('#learning-complete-btn');
    if (!completeBtn.length) return;

    var activeItem = $('.tab-item.active');
    var toggleCheckbox = activeItem.find('.js-passed-lesson-toggle');
    ...
    var isCompleted = toggleCheckbox.is(':checked');
    var buttonText = completeBtn.find('.js-text');

    var completedText = (typeof completedLang !== 'undefined') ? completedLang : 'مكتمل';
    var markAsCompletedText = (typeof markAsCompletedLang !== 'undefined') ? markAsCompletedLang : 'وضع علامة مكتمل';

    if (isCompleted) {
        // **الحالة: الدرس مكتمل**
        buttonText.text(completedText);
        // 1. تغيير اللون إلى الأخضر
        completeBtn.removeClass('btn-success').addClass('btn-secondary');
        // 2. تعطيل الزر لمنع الضغط عليه مرة أخرى
        completeBtn.prop('disabled', true);
    } else {
        // **الحالة: الدرس غير مكتمل**
        buttonText.text(markAsCompletedText);
        // 1. إظهار الزر باللون الأساسي (مثل باقي الأزرار)
        completeBtn.removeClass('btn-success').addClass('btn-primary');
        // 2. التأكد من أن الزر فعال وقابل للنقر
        completeBtn.prop('disabled', false);
    }
}

/**
 * معالج النقر على أزرار التنقل (التالي والسابق)
 */
$('body').on('click', '#learning-previous-btn, #learning-next-btn', function() {
    var $this = $(this);
    var id = $this.attr('data-id');
    var type = $this.attr('data-type');

    if (id && type) {
        var targetItem = $('.tab-item[data-id="' + id + '"][data-type="' + type + '"]');
...
/**
 * معالج النقر على زر إكمال الدرس (نسخة معدلة)
 */
$('body').on('click', '#learning-complete-btn', function() {
    // التأكد من أن الزر ليس معطلاً قبل تنفيذ أي شيء
    if ($(this).is(':disabled')) {
        return;
    }
    ...
    // التأكد من أن مربع الاختيار موجود وغير محدد بالفعل
    if (toggleCheckbox.length && !toggleCheckbox.is(':checked')) {
        // تحديد مربع الاختيار وتفعيل حدث التغيير لإرسال الطلب
        toggleCheckbox.prop('checked', true).trigger('change');
        
        // تحديث حالة الزر فورًا لتحسين تجربة المستخدم
        setTimeout(updateLessonCompleteButton, 100);
    }
});
