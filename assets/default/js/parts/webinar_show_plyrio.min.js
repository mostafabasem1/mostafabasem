(function ($) {
  ...
  /**
  /**
 * webinar demo modal with Plyr.io player
 */

var courseDemoPlayer; // تم تغيير الاسم ليعكس استخدام Plyr

$('body').on('click', '#webinarDemoVideoBtn', function (e) {
    e.preventDefault();

    // 1. تدمير المشغل السابق إن وجد لمنع تراكم المشغلات في الذاكرة
    if (courseDemoPlayer && typeof courseDemoPlayer.destroy === 'function') {
        courseDemoPlayer.destroy();
    }
    ...
    // 2. تجهيز بنية HTML المناسبة لمشغل Plyr بناءً على مصدر الفيديو
    if (source === 'youtube' || source === 'vimeo') {
        // Plyr يستخدم عنصر div مع بيانات خاصة ليوتيوب وفيميو
        // يجب أن يحتوي 'path' على معرّف الفيديو فقط (e.g., 'bTqVqk7FSmY')
        playerHtml = `<div id="${videoTagId}" data-plyr-provider="${source}" data-plyr-embed-id="${path}"></div>`;
    } else {
        // Plyr يستخدم وسم <video> لمقاطع الفيديو المستضافة ذاتيًا
        // يجب أن يحتوي 'path' على الرابط الكامل لملف الفيديو (e.g., '/uploads/video.mp4')
        playerHtml = `
            <video id="${videoTagId}" playsinline controls>
    ...
    Swal.fire({
        ...
        onOpen: function onOpen() {
            // 3. تهيئة مشغل Plyr الجديد
            courseDemoPlayer = new Plyr('#' + videoTagId);
        },
        onClose: function onClose() {
            // 4. (ممارسة جيدة) تدمير المشغل عند إغلاق النافذة المنبثقة
             if (courseDemoPlayer && typeof courseDemoPlayer.destroy === 'function') {
                courseDemoPlayer.destroy();
            }
        }
  ...
  $('body').on('click', '.js-course-add-to-cart-btn', function (e) {
    var $this = $(this);
    $this.addClass('loadingbar primary').prop('disabled', true);  // إضافة تأثير التحميل
    var $form = $this.closest('form');
    var formData = $form.serialize();  // جمع البيانات من النموذج

    $.ajax({
        url: '/cart/store',  // رابط الإرسال
        method: 'POST',
        data: formData,
        success: function(response) {
        
            // إعادة التوجيه إلى صفحة السلة بعد نجاح الإضافة
            window.location.href = '/public/cart';
        },
        error: function() {
            // في حال حدوث خطأ أثناء الإرسال، أعد تفعيل الزر وأزل التأثيرات
            $this.removeClass('loadingbar primary').prop('disabled', false);
            alert('حدث خطأ أثناء إضافة الدورة إلى السلة. حاول مرة أخرى.');
        }
    });
});
