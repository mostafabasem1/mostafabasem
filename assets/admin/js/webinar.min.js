...
(function ($) {
  ...
  $('body').on('click', '.js-add-course-content-btn, .add-new-interactive-file-btn', function (e) {
    ...
    // نفس منطق الكود الأصلي لتحديد الفورم ونسخ المحتوى
    switch (type) {
        case 'file':
            $formTemplate = $('#newFileForm');
            $formTemplate.find('.chapter-input').val(chapterId);
            html = $formTemplate.html();
            break;

        case 'new_interactive_file':
            $formTemplate = $('#newInteractiveFileForm');
            $formTemplate.find('.chapter-input').val(chapterId);
            html = $formTemplate.html();
            break;

        case 'session':
            $formTemplate = $('#newSessionForm');
            $formTemplate.find('.chapter-input').val(chapterId);
            html = $formTemplate.html();
            break;

        case 'text_lesson':
            $formTemplate = $('#newTextLessonForm');
            $formTemplate.find('.chapter-input').val(chapterId);
            html = $formTemplate.html();
            break;

        case 'assignment':
            $formTemplate = $('#newAssignmentForm');
            $formTemplate.find('.chapter-input').val(chapterId);
            html = $formTemplate.html();
            break;

        case 'quiz':
            $formTemplate = $('#newQuizForm');
            $formTemplate.find('.chapter-input').val(chapterId);
            html = $formTemplate.html();
            break;
    }
    ...
    // بدلاً من إضافة الفورم للصفحة، سنضيفه للنافذة المنبثقة
    if (html !== '') {
        // نستبدل كلمة record بالـ key العشوائي
        html = html.replace(/record/g, key);

        var $modal = $('#addContentModal');

        // نضع عنوان النافذة والمحتوى المنسوخ بداخلها
        $modal.find('.modal-title').text($this.text().trim());
        $modal.find('.modal-body').html(html);

        // نعرض النافذة المنبثقة
        $modal.modal('show');

        // بعد عرض النافذة، نقوم بتفعيل الإضافات اللازمة
        $modal.off('shown.bs.modal').on('shown.bs.modal', function () {
            ...
            // كود خاص بتفعيل محرر النصوص والـ select2 لنوع الدرس النصي
            if (type === 'text_lesson') {
                var summernoteTarget = $modal.find('.js-content-summernote');
                var hiddenSummernoteTarget = $modal.find('.js-hidden-content-summernote');
                var selectTarget = $modal.find('.attachments-select2');

                // تعديل الـ class ليكون فريداً باستخدام الـ key
                summernoteTarget.attr('class', 'js-content-summernote-' + key);
                hiddenSummernoteTarget.attr('class', 'js-hidden-content-summernote-' + key);
                selectTarget.attr('class', 'attachments-select2-' + key);
...
function handleShowFileInputsBySource($form, source, fileType) {
    ...
    // استخدام find مع تحديد السياق للفورم الحالي فقط
    var $fileTypeVolumeInputs = $form.find('.js-file-type-volume').first();
    var $volumeInputs = $form.find('.js-file-volume-field').first();
    var $downloadableInput = $form.find('.js-downloadable-input').first();
    var $onlineViewerInput = $form.find('.js-online_viewer-input').first();
    var $filePathInputGroup = $form.find('.js-file-path-input').first();
    var $s3FilePathInputGroup = $form.find('.js-s3-file-path-input').first();
    var $filePathButton = $filePathInputGroup.find('button').first();
    var $filePathInput = $filePathInputGroup.find('input').first();
  ...
  $('body').on('change', '.js-file-storage', function (e) {
    ...
    // استخدام closest للحصول على الفورم الأقرب فقط
    var formGroup = $(this).closest('.file-form');
    var fileType = formGroup.find('.js-ajax-file_type').first().val();
  ...
  $('body').on('change', '.js-ajax-file_type', function (e) {
    ...
    // استخدام closest للحصول على الفورم الأقرب فقط
    var formGroup = $(this).closest('.file-form');
    var source = formGroup.find('.js-file-storage').first().val();
